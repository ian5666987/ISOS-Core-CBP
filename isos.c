...
struct IsosClock IsosMainClock;
struct IsosClock LastSchedulerRun;
struct IsosClock LastSchedulerFinished; //probably good for debugging
struct IsosClock SchedulerPeriod;
struct IsosTask IsosTaskList[MAX_TASK_SIZE];
struct IsosDueTask IsosDueTaskList[MAX_TASK_SIZE]; //an array, as required by the QuickSort
...
enum IsosResourceTaskType LastClaimedResourceTask = IsosResourceTaskType_Unspecified; //Used as a flag if there is any resource task that has just been claimed
enum IsosResourceTaskType LastReleasedResourceTask = IsosResourceTaskType_Unspecified; //Used as a flag if there is any resource task that has just been released
...
static struct IsosTask* genericTask;
static struct IsosTaskInfo* genericTaskInfo;
static struct IsosTaskActionInfo* genericTaskActionInfo;
...
void Isos_initClockToNow(struct IsosTaskInfo* taskInfo){
...
void Isos_queueOnDueHandled(struct IsosTaskInfo* taskInfo, struct IsosClock* clock){
...
void Isos_queueOnDue(struct IsosTaskInfo* taskInfo, struct IsosClock clock){
...
void Isos_removeDueTaskByIndex(short dueTaskIndex){
    ...
    if (dueTaskIndex < IsosDueTaskSize - 1) //NOT the last element
        ...
        memcpy(&IsosDueTaskList[dueTaskIndex], &IsosDueTaskList[dueTaskIndex + 1], sizeof(struct IsosDueTask) * (IsosDueTaskSize - (dueTaskIndex + 1)));
...
void Isos_insertTaskOnDue(short currentRunningTaskIndex, struct IsosTaskInfo* taskInfo, struct IsosClock clock){
    short prevIndex;
    short tailTaskNo;
    ...
    if (currentRunningTaskIndex == prevIndex){ //simply shift the current one to the next position
        ...
        memcpy(&IsosDueTaskList[currentRunningTaskIndex + 1], &IsosDueTaskList[currentRunningTaskIndex], sizeof(struct IsosDueTask) * tailTaskNo);
...
void Isos_prepareToDueTask(struct IsosTaskInfo* taskInfo, unsigned char priority, char withReset){
...
void Isos_commonPrepareDueNonCyclicalTask(struct IsosTaskInfo* taskInfo, unsigned char priority, char withReset, struct IsosClock clock){
...
void Isos_execute(struct IsosTask* task){
    struct IsosClock clock;
...
char Isos_registerTask(enum IsosTaskType type, enum IsosResourceTaskType resourceType, char isCyclical, char enabled,
                       short timeInfoDay, long timeInfoMs, unsigned char priority, void (*taskAction)(unsigned char, struct IsosTaskActionInfo*)){
    struct IsosClock timeInfo;
    struct IsosTask task;
...
struct IsosClock Isos_GetClock(){ return IsosMainClock; } //create a copy of main clock for use
...
void Isos_handleLastReleasedResource(short* currentDueIndex){
    ...
    static struct IsosTask* nextClaimerTask;
...
void Isos_Run(){
    ...
    //Because there are many variables initialized here, static could probably help to save some initialization time
    static struct IsosClock measuredClock, clock;
    static short initialDueTaskSize, i;
    static struct IsosDueTask* dueTask;
...
void Isos_solveCompetingNextClaims(unsigned char* resourceTaskInfoFlags, unsigned char challengerId, unsigned char challengerPriority){
    unsigned char hasCompetitor;
    unsigned char currentPriority;
...
void Isos_Wait(unsigned char taskId, short waitingDay, long waitingMs){
    struct IsosClock clock, addClock;
